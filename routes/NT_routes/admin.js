  ...
  try {
    ...
    res
      ...
      .send({
        admin: newadmin,
        token: token,
        status: "Admin Account Creation Success",
      });
  } catch (error) {
...
router.put("/update", auth, async (req, res) => {
  res.header("Access-Control-Allow-Origin", "http://localhost:3000");
  res.header("Access-Control-Allow-Credentials", "true");
  const { fname, lname, email, username, nic, description, profileImage } =
    req.body;
  try {
    const updateValus = {
      fname: fname,
    ...
    res
      .status(200)
      .send({ status: "Admin Profile Updated", admin1: adminUpdate });
  } catch (error) {
    res.status(500).send({ error: error.message });
    console.log(error);
  }
