...
router.post("/add/:id", auth, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ status: "Added to Wishlist", wishlist: wishlistItem });
  } catch (error) {
...
router.get("/display", auth, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ status: "wishlist retrieved", wishlist: req.Cus.wishList });
  } catch (error) {
...
router.delete("/delete/:id", auth, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ status: "product removed from the list", wishlist: deleteItem });
  } catch (error) {
    res
      .status(500)
