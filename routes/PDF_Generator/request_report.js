...
router.post("/generateRequestReport", async function (req, res, next) {
  ...
  const requestReportRequest = req.body.request;
  console.log(requestReportRequest);
  ...
  myDoc.on("end", () => {
    ...
    res
      .writeHead(200, {
        ...
        "Content-disposition": `attachment;filename=MyRequest_${datestamp}.pdf`,
      })
      .end(pdfData);
    // console.log(pdfData)
    //console.log(res)
  ...
  const table = {
    headers: ["Request Code", "Item Name", "Brand", "Model", "Version"],
    rows: [],
  };

  for (const request of requestReportRequest) {
    table.rows.push([
      request._id,
      request.itemname,
      request.brand,
