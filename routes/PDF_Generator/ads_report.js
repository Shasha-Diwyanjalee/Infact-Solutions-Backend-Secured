...
router.post("/generateadsreport", async function (req, res, next) {
  ...
  const adsReportRequest = req.body.ads;
  console.log(adsReportRequest);
  ...
  myDoc.on("end", () => {
    ...
    res
      .writeHead(200, {
        ...
        "Content-disposition": `attachment;filename=Advertisementslist_${datestamp}.pdf`,
      })
      .end(pdfData);
    // console.log(pdfData)
    //console.log(res)
  ...
  const table = {
    headers: ["Title", "PublishDate", "Description"],
    rows: [],
  };

  for (const adsItem of adsReportRequest) {
    table.rows.push([
      adsItem.title,
      adsItem.publishdate,
      adsItem.description,
